    ...
    do {
        ...
        switch(cmd) {
            ...
            case CONNECT: {
                ...
                if (aMachine < 0 || bMachine < 0) {
                    error_code = NO_SUCH_MACHINE;
                    break;
                }
                int aType = network[aMachine]->myType();
                int bType = network[bMachine]->myType();
                if (network[aMachine]->canAcceptConnection(bType) && network[bMachine]->canAcceptConnection(aType)) {
                    network[aMachine]->connect(ip2, bType);
                    network[bMachine]->connect(ip1, aType);
                }
                else
